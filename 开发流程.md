...
获取每个房间数据库表的数据
赋值给本地模型用来缓存
便利本地模型数据，获取打勾的个数（实践证明便利不了，只能一个个搞）

把打勾个数一个个累加起来，除以所有需要清理掉位置个数
将上面的数据写到标签里面，写到进度条里面


 viewWillAppear 记录写入UI
 viewDidAppear 清除成员变量
1.

Property access result unused - getters should not be used for side effects

Collection expression type 'Kitchen *' may not respond to 'countByEnumeratingWithState:objects:count:'
